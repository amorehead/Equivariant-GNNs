...
class LitGraphSAGE(pl.LightningModule):
    ...
    def forward(self, graph1, graph2):
        ...
        graph1_feats = self.gnn_forward(graph1)
        graph2_feats = self.gnn_forward(graph2)
...
def cli_main():
    ...
    # Initialize logger
    args.experiment_name = f'LitGraphSAGE-l{args.num_hidden_layers}-h{args.hidden_dim}-b{args.batch_size}' \
    ...
    args.ckpt_name = args.ckpt_name if args.ckpt_name is not None else 'LitGraphSAGE-{epoch:02d}-{val_auroc:.2f}.ckpt'
    ...
    checkpoint_callback = ModelCheckpoint(monitor='val_auroc', save_top_k=3, dirpath=args.ckpt_dir,
                                          filename='LitGraphSAGE-{epoch:02d}-{val_auroc:.2f}')

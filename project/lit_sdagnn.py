...
class LitSDAGNN(pl.LightningModule):
    """An siamese Deep Adaptive Graph Neural Network (DAGNN)."""
    ...
    def __init__(self, node_feat: int = 29, num_classes: int = 2, num_hidden_layers: int = 12, hidden_dim: int = 32,
        ...
        """Initialize all the parameters for a siamese DAGNN."""
        super().__init__()
    ...
    def build_gcn_model(self):
        """Define the MLP layers of a single DAGNN."""
    ...
    def forward(self, graph1, graph2):
        ...
        # Forward propagate with both GCNs - must add self loops to avoid exploding logit values
        graph1_feats = self.gcn_forward(graph1.add_self_loop())
        graph2_feats = self.gcn_forward(graph2.add_self_loop())
...
def cli_main():
    ...
    # Data
    # -----------
    # dips_data_module = DIPSDGLDataModule(data_dir=args.dips_data_dir,
    #                                      batch_size=args.batch_size,
    #                                      num_dataloader_workers=args.num_workers)
    # dips_data_module.prepare_data()
    # dips_data_module.setup()

    db5_data_module = DB5DGLDataModule(data_dir=args.db5_data_dir,
                                       batch_size=args.batch_size,
                                       num_dataloader_workers=args.num_workers,
                                       full=True)
    db5_data_module.prepare_data()
    db5_data_module.setup()

    # ------------
    # Model
    # ------------
    lit_sdagnn = LitSDAGNN(
        node_feat=db5_data_module.db5_test.num_features,
    ...
    # Initialize logger
    args.experiment_name = f'SDAGNN-l{args.num_hidden_layers}-h{args.hidden_dim}-b{args.batch_size}' \
    ...
    args.ckpt_name = args.ckpt_name if args.ckpt_name is not None else 'LitSDAGNN-{epoch:02d}-{val_auroc:.2f}.ckpt'
    ...
    checkpoint_callback = ModelCheckpoint(monitor='val_auroc', save_top_k=3, dirpath=args.ckpt_dir,
                                          filename='LitSDAGNN-{epoch:02d}-{val_auroc:.2f}')
    ...
    trainer.fit(lit_sdagnn, datamodule=db5_data_module)

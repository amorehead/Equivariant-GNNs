...
from project.utils.from_se3cnn.SO3 import torch_default_dtype, irr_repr
from project.utils.from_se3cnn.cache_file import cached_dirpklgz
from project.utils.from_se3cnn.representations import SphericalHarmonics
...
def _basis_transformation_Q_J(J, order_in, order_out, version=3):  # pylint: disable=W0613
    ...
    with torch_default_dtype(torch.float64):
        def _R_tensor(a, b, c): return kron(irr_repr(order_out, a, b, c),
        ...
        assert all(
            torch.allclose(_R_tensor(a, b, c) @ Q_J, Q_J @ irr_repr(J, a, b, c)) for
            a, b, c in torch.rand(4, 3))
...
def spherical_harmonics(order, alpha, beta, dtype=None):
    ...
    return src.project.utils.from_se3cnn.representations.SphericalHarmonics.get(order, theta=math.pi - beta, phi=alpha)
